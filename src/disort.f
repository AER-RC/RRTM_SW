...
c     .. Parameters ..
     ...
     &          NNLYRI = MXCMU*MXCLY )
c ejm - original
c      PARAMETER ( MXCLY = 31, MXULV = 32, MXCMU = 128, MXUMU = 10,
...
c                               Be sure SLFTST sets all print flags off.
   ...
   10 CONTINUE
     ...
     &    'Discrete Ordinates Radiative Transfer Program, Version 1.1',
     &    HEADER
...
c> call to PLKAVG replaced by PLNK0 for monochromatic calculations
c> when WVNMHI is less than or equal to WVNMLO
          TPLANK = TEMIS * PLNK0( WVNMLO, TTEMP )
          BPLANK =         PLNK0( WVNMLO, BTEMP )
          DO 31  LEV = 0, NLYR
            PKAG( LEV ) = PLNK0( WVNMLO, TEMPER(LEV) )
  31     CONTINUE
        END IF
...
c                                    ** Azimuth-independent case
      ...
      iazflg=0
      do iu = 1, numu
         if(1.-abs(umu(iu)).gt.1.e-5 )iazflg=1
      end do
...
c                      ** from A*B because A*B may (over/under)flow
     ...
     &       ( A.LT.0.0 .AND. B.GT.0.0 ) ) RATIO = -RATIO
      ...
      real function plnk0(v,temp)
c>
c> function plnk0 returns planck function at wavenumber v
c> in fascode units (watts/cm**2/ster./cm-1)
c>
      data c1,c2/1.1910620e-12,1.438786314232/
c>
      den=exp(c2*v/temp)-1.
      plnk0=c1*v**3/den
c>
